...
class app:
    def __init__(self):
        if(os.geteuid() and os.name == 'posix'):
            sys.exit(print('Warning: Please run app in sudo'))
        self.__metabase = {
            'actions': {},
    ...
    def init(self, initial_func):
        self.__on_init = True
        self.__metabase['actions']['initial'] = initial_func
        initial_func()
        self.__on_init = False
        ...
        if os.environ.get('RUN_MAIN') != 'true':
            ...
            with open(settings_py_path, 'w+') as settings_py:
                if 'import os' not in settings_py_data:
                    settings_py_data = settings_py_data.replace(
                ...
                if 'yaplee_dir' not in settings_py_data:
                    settings_py_data = settings_py_data.replace(
                        "'DIRS': [],",
                        "'DIRS': [os.environ.get('yaplee_dir')],"
                    )
